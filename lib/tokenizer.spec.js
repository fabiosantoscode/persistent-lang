...
it('ignores comments', () => {
  expect(getTokens`# this is a comment`).toMatchInlineSnapshot(`end`)
  expect(getTokens`1 # comment`).toMatchInlineSnapshot(`number 1; end`)
  expect(getTokens`before # comment\nafter`).toMatchInlineSnapshot(
    `word before; word after; end`
  )
})
