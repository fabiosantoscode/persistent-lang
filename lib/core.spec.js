...
import { createProgram, evaluate, satisfyYield } from './core.js'

const advanceToPause = (program, state) => {
  let execLimit = 100
...
const getAllYielded = (code) => {
  ...
  let execLimit = 100
  while (state.runStatus === 'running') {
    advanceToPause(program, state)

    if (state.runStatus === 'yielded') {
      yieldedList.push(state.yielded)
